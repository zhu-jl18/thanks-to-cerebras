# Cerebras KV 持久化部署指南（v4 模型池轮询版）

## 核心要点
- 服务入口是 `deno-v4.ts`（推荐；`deno-v3.ts` 为旧版单默认模型）。
- 所有密钥与配置持久化在 Deno KV；代理转发的热路径使用内存缓存做 Key/Model 轮询，KV 只负责后台持久化（默认异步刷盘，最终一致）。
...
- 访问日志页面，应看到类似输出：
  ```
  🚀 Cerebras 密钥管理系统启动 (KV 持久化版 v4 - 模型池轮询)
  - 管理面板: 主页
...
- **统计刷盘**：默认每 15000ms 将 `useCount/lastUsed/totalRequests/currentModelIndex` 异步写回 KV，可用环境变量 `KV_FLUSH_INTERVAL_MS` 覆盖；设为 `0` 可关闭刷盘（统计将不再持久化）。
- **模型池管理**：在“模型池”区域增删/测试模型后，会立即写入 KV；对外 `/v1/models` 仅返回一个虚拟模型名，实际转发会按模型池轮询。
- **面板一致性**：`/` 与 `/api/stats` 每次从 KV 读取，避免 Deno Deploy 多实例/多 isolate 时出现面板数据跳变；但统计数值仍受刷盘间隔影响（最终一致）。
...
- **模型池更新无效**：强制刷新浏览器缓存后重试；若仍失败检查日志与 `/api/config`、`/api/models` 输出是否一致。
