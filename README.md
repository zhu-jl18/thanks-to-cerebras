...
提供四个版本供选择：基础版、增强版（支持鉴权+模型默认映射）、Ultra v3（KV 面板，单默认模型）以及 Ultra v4（KV 面板 + 模型池轮询，推荐）
...
| Ultra v4（KV 面板 + 模型池轮询，推荐） | `deno-v4.ts` | 想要面板管理 key 池/模型池，追求 0 等待 | Deno KV（面板持久化） | ❌ 默认关闭（直通，宁可 429） | ✅ 有（对内按模型池轮询） | ✅ |
| Ultra v3（KV 面板，单默认模型） | `deno-v3.ts` | 想要面板管理 key 池/默认模型，追求 0 等待 | Deno KV（面板持久化） | ❌ 默认关闭（直通，宁可 429） | ✅ 有（映射到面板默认模型） | ✅ |
